...
import models.model_vqa_BLIP as model_vqa_BLIP
...
def evaluation(model, data_loader, tokenizer, device, config) :
    ...
    for n, (image, question, question_id) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):        
        ...
        output = model(image, question, answer_list, train=False, k=config['k_test'])      
        
        for ques_id in zip(question_id):
            ques_id = int(ques_id.item())          
            result.append({"question_id":ques_id, "answer":output})   
