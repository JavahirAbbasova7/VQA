...
class ALBEF(nn.Module):
    ...
    def patch_alignment(self, visual_patch_proj, text_cls_proj): # shapes =  [B, 196, 768], [B, 1, 768]
    ...
    def forward(self, image, text, alpha=0):
        ...
        image_feat = self.vision_proj(image_embeds[:,1:,:])
        ...
        with torch.no_grad():
            ...
            image_feat_m = self.vision_proj_m(image_embeds_m[:,1:,:])                                
            text_output_m = self.text_encoder_m.bert(text.input_ids, attention_mask = text.attention_mask,                      
                                                return_dict = True, mode = 'text')    
            text_feat_m = F.normalize(self.text_proj_m(text_output_m.last_hidden_state[:,0,:]),dim=-1)
            text_feat_all = torch.cat([text_feat_m.t(),self.text_queue.clone().detach()],dim=1)

            patch_activations_m = self.patch_alignment(image_feat_m, text_feat_m) # shapes =  [B, 196]
            patch_pooled_visual_projections_m = torch.sum(image_feat_m * patch_activations_m.unsqueeze(-1), dim=1) # [B, 768]
            norm_img_feat_m = F.normalize(patch_pooled_visual_projections_m, dim=-1)
            image_feat_all = torch.cat([norm_img_feat_m.t(),self.image_queue.clone().detach()],dim=1)     

            sim_i2t_m = norm_img_feat_m @ text_feat_all / self.temp 
        ...
        patch_activations = self.patch_alignment(image_feat, text_feat) # shapes =  [B, 196]
        ...
        sim_i2t = norm_img_feat @ text_feat_all / self.temp 
        ...
        self._dequeue_and_enqueue(norm_img_feat_m, text_feat_m)
